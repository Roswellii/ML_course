...
# 高斯核函数
c_param= 10000
prob= svm.svm_problem(data_y, data_x, isKernel=True) # 导入数据
param= svm.svm_parameter('-t 0 -c %d -q' % c_param) # 设定参数，线性核函数，代入C
model= svmutil.svm_train(prob, param, '-q') # 建立模型
support_vectors = model.get_SV()
p_label,p_acc,p_val= svmutil.svm_predict(data_y, data_x, model) # 进行预测
